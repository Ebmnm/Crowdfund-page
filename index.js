...
function progressBar() {
  if (parseInt(moneyRaised.innerHTML.slice(1).replace(/,/g, ""), 10) < 100000) {
    progress.style.width = `${
      parseInt(moneyRaised.innerHTML.slice(1).replace(/,/g, ""), 10) / 1000
    }%`;
  } else if (
    parseInt(moneyRaised.innerHTML.slice(1).replace(/,/g, ""), 10) > 100000
  ) {
...
function updatePledge() {
  ...
  if (radioInputs[3].checked == true) {
    ...
    if (mahogany.innerHTML == 0) {
      ...
      mahogany2.forEach(
        (mahogany2) => (mahogany2.innerHTML = mahogany.innerHTML)
      );
    }
  }
}
...
continueButton.addEventListener("click", () => {
  ...
  popup.classList.remove("active");
  let newMoneyRaised =
    parseInt(raisednumber.replace(/,/g, ""), 10) + parseInt(amountInput.value);
